...
export async function POST(req: NextRequest) {
    try {
        ...
        const { prompt, systemPrompt, model = 'llama-3.3-70b-versatile', temperature = 0.7, maxTokens = 1024 } = body
        ...
        const startStreaming = async () => {
            try {
                if (isOpenAI) {
                    ...
                    await writer.write(encoder.encode(`data: ${JSON.stringify({ done: true, usage: clientUsage })}\n\n`))
                    ...
                    for await (const chunk of responseStream) {
                        ...
                        const xGroq = (chunk as unknown as Record<string, unknown>).x_groq as { usage?: OpenAI.Completions.CompletionUsage } | undefined
                        if (xGroq?.usage) {
                            usage = xGroq.usage
                        }
                    }
